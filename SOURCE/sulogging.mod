...
IMPORT DID, Strings, CommonSettings, DomainEditor;
...
FROM RINIData IMPORT
    (* proc *)  OpenINIFile, CloseINIFile,
                RemoteOperation, INIPut, INIPutString,
...
VAR
    ...
    OldTransLevel: CARDINAL;
    OldEnableCommonLog: BOOLEAN;
    OldCommonLogName, OldTransLogName, OldSyslogHost: FilenameString;
...
PROCEDURE SetLanguage (lang: LangHandle);
    ...
    BEGIN
        ...
        StrToBuffer (lang, "Logging.CommonLog", stringval);
        OS2.WinSetDlgItemText (pagehandle, DID.EnableCommonLog, stringval);
...
PROCEDURE EnableFields (hwnd: OS2.HWND);
    ...
    BEGIN
        IF QueryButton (hwnd, DID.EnableCommonLog) > 0 THEN
...
PROCEDURE LoadValues (hwnd: OS2.HWND);
    ...
    BEGIN
        ...
        IF NOT INIFetch ('$SYS', 'EnableCommonLog', OldEnableCommonLog) THEN
            OldEnableCommonLog := FALSE;
        END (*IF*);
        OS2.WinSendDlgItemMsg (hwnd, DID.EnableCommonLog, OS2.BM_SETCHECK,
                                 OS2.MPFROMSHORT(ORD(OldEnableCommonLog)), NIL);

        IF INIGetString ('$SYS', 'CommonLogName', name) THEN
            OldCommonLogName := name;
        ELSE
            OldCommonLogName := '';
            name := 'COMMON.LOG';
        END (*IF*);
        OS2.WinSetDlgItemText (hwnd, DID.CommonLogName, name);
...
PROCEDURE StoreData;
    ...
    VAR val: CARDINAL;  bool: BOOLEAN;  hwnd: OS2.HINI;
    ...
    BEGIN
        ...
        bool := QueryButton (hwnd, DID.EnableCommonLog) > 0;
        IF bool <> OldEnableCommonLog THEN
            INIPut ('$SYS', 'EnableCommonLog', bool);
        END (*IF*);

        OS2.WinQueryDlgItemText (hwnd, DID.CommonLogName, TSIZE(FilenameString),
                                                                 filename);
        ...
        val := QueryButton (hwnd, DID.DiskLog)
        ...
        IF val <> OldTransLevel THEN
            INIPut ('$SYS', 'TransLevel', val);
        END (*IF*);
